...
def fis_opt(e_teta, error, params=[], grafica=False):
    ...
    e_teta_med_neg = fuzz.trimf(x_e_teta, [-1-b*factor_apertura,-1, -1+b*factor_apertura])
    e_teta_lo      = fuzz.trimf(x_e_teta, [0-c*factor_apertura, 0, 0+c*factor_apertura])
    e_teta_med_pos = fuzz.trimf(x_e_teta, [1-d*factor_apertura, 1, 1+d*factor_apertura])
    e_teta_hi_pos = fuzz.trapmf(x_e_teta, [1, 5-(a*2), 100, 100])

    error_hi_neg  = fuzz.trapmf(x_error, [-100, -100, -5+(e*2), -1])
    error_med_neg = fuzz.trimf(x_e_teta, [-1*factor_apertura-f,-1, -1+f*factor_apertura])
    error_lo      = fuzz.trimf(x_error,  [0-g*factor_apertura, 0, 0+g*factor_apertura])
    error_med_pos = fuzz.trimf(x_e_teta, [1-h*factor_apertura, 1, 1+h*factor_apertura])
