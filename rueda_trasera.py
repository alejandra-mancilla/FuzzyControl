...
plt.plot(ruta_referencia.X(spline),ruta_referencia.Y(spline), "-r", label= "Ruta : Curva Spline")
plt.plot(x, y, "-g", label= "Movimiento Robot")
plt.axis("equal")
plt.grid(True)
plt.xlabel("X(mts)")
plt.ylabel("Y(mts)")
...
# plt.ylabel("teta (grados)")
# plt.legend()
#
# plt.subplots(1)
# plt.plot(spline, np.rad2deg(ruta_referencia.calc_yaw(spline)),"-r", label="yaw")
# plt.grid(True)
